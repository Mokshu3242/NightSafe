...
All public. Download into `Data/raw/` (also mirrored on Google Drive — https://drive.google.com/drive/folders/1OQntLM_ac7X8NBTJKDf0l7CoWN5IJ8Fv?usp=drive_link).
...
└── Data/processed/               # generated by the pipeline
    ├── feature_table_temporal.csv   # phase1 output
    ├── corridor_forecast.csv        # phase2 output (scores + fairness)
    ├── corridor_rankings.csv        # phase3 output (dashboard reads this)
    ├── shap_reasons_by_street.csv   # phase4 output (per-corridor reasons)
    ├── feature_importance.csv, fairness_audit.csv, model_metrics_corrected.csv
    ├── model_*.pkl, scaler.pkl, ensemble_weights.pkl
    └── philadelphia_roads.geojson, help_locations.geojson
```
...
python fetch_census_income.py     # needs a free Census API key in .env
...
python phase4_shap.py             # SHAP explanations
